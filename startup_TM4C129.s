...
Reset_Handler   PROC
				...
				;call _systemcall_table_init
				LDR		R0, =_systemcall_table_init
				BLX		R0
				
				; add some code to call _kinit
				LDR		R0, =_kinit
				BLX		R0
...
SVC_Handler     PROC
				...
				;call _systemcall_table_jump      USE R7 (R7 holds the value of which system call to call)
				PUSH 	{lr}		; save lr
				LDR		R2, =_systemcall_table_jump
				BLX		R2
				
				; do not update the following two lines change from handler stack to user stack
                MRS	R1, PSP						; added for CSS422 project
				STR	R0, [R1]					; added for CSS422 project
				
				; add some code here to return from _systemcall_table_jump
				POP 	{lr}		; resume lr
				BX		lr	
                ENDP
DebugMon_Handler\
